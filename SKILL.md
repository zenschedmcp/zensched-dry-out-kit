...
Then `UPDATE settings SET value = '<form_id>' WHERE key = 'moisture_form_id';`. Attach it to every job's event with `form_assign(form_id, event_id=<event_id>, idempotency_key="assign-moisture-{event_id}")` **before** the first `shift_create` on that event, so the shift installs the form on the phone. Re-assign after every event roll. If a shift was created before the assign, `form_assign(event_id=...)` installs the form on the existing shifts on that event — never cancel and recreate the shift for this (the recreate would reuse the same `shift-job-{job_id}-{YYYYMMDD}` idempotency key and replay the cancelled shift).

Submission `data` comes back keyed by the identifiers above. Select values are **option keys** (lowercase, non-alphanumerics → `_`, truncated at 30 characters): `room` ∈ `living_room`, `dining_room`, `kitchen`, `bedroom_1`, `bedroom_2`, `bedroom_3`, `bathroom_1`, `bathroom_2`, `hallway`, `basement`, `garage`, `laundry`, `closet`, `other`; `meter_type` ∈ `pin`, `pinless`, `thermo_hygrometer`, `other`. `moisture_pct` is a number. Store the raw keys. Photos come back in the submission's `media` list (each entry has `cdn_url`, `thumbnail_url`, `original_filename`, and a numeric `field_id`; the Moisture Log's only photo field is the meter, so every entry is a meter photo); copy the `cdn_url`s into that room's object in `moisture_readings`. A submission with photos bills $0.15 instead of $0.05, **once ever**.
...
4. Build `moisture_readings` as a JSON array, one object per submission: `room` (option key), `moisture_pct`, `meter_type` (option key), `photo_urls` (every `cdn_url` in that submission's `media`; the meter is the form's only photo field), `notes`, `submission_id`. Count `rooms_logged`. For `rooms_still_wet`, compare each reading to that room's `target_moisture_pct` (else the setting).
